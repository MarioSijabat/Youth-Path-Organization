...
<article class="container mt-28">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 pt-8">
        ...
        <div class="py-[26px] px-6 rounded-3xl bg-transparent drop-shadow-custom">
            <div class="animate-blob-blue absolute w-[400px] h-[60px] bg-customBlue rounded-full filter blur-md-custom -z-10"></div>

            <a href="{{ route('pengumuman.show', $announcement->slug) }}"
                class="text-base sm:text-lg font-montserrat font-semibold text-primary-blackCustom max-h-[5em] line-clamp-2 overflow-hidden whitespace-normal mb-4">
                {{ $announcement->title }}
            </a>
            <p class="font-montserrat text-xs sm:text-sm font-semibold text-secondary-blackCustom mb-1.5 text-ellipsis whitespace-normal">
