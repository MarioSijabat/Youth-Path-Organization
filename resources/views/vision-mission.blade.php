...
@vite('resources/css/app.css')
    ...
    <article class="container mt-28">
      ...
      <div class="relative grid grid-cols-1 sm:grid-cols-2 gap-15 my-15 mb-20">
        ...
        <!-- Blue Separator Between VISION and MISSION (Positioned Absolutely) -->
      ...
      >
        ...
        <div class="p-[30px]">
          ...
          >
          We empower youth with the knowledge, skills, and opportunities to become leaders in sustainability and social change. Education is the foundation for informed action, enabling youth to make a lasting impact.
          </p>
        </div>
        <div>
            @if(isset($images[0]))
              <div class="p-[30px] sm:col-span-5 relative overflow-hidden rounded-3xl">
                <img src="{{ asset('storage/'. $images[0]) }}" alt="Innovative" class="w-full object-cover rounded-2xl" />
              </div>
            @else
                <p class="text-secondary-whiteCustom p-[30px]">No Image Available</p>
            @endif
        </div>
        <div class="p-[30px]">
          <h2
        ...
        <div>
            @if(isset($images[1]))
              <div class="p-[30px] sm:col-span-5 relative overflow-hidden">
                <img src="{{ asset('storage/'. $images[1]) }}" alt="Collaboration" class="w-full object-cover rounded-2xl" />
              </div>            
            @else
                <p class="text-secondary-whiteCustom p-[30px]">No Image Available</p>
            @endif
        ...
        <div>
            @if(isset($images[2]))
              <div class="p-[30px] sm:col-span-5 relative overflow-hidden rounded-3xl">
                  <img src="{{ asset('storage/'. $images[2]) }}" alt="Action" class="w-full object-cover rounded-2xl" />
              </div>            
            @else
                <p class="text-secondary-whiteCustom p-[30px]">No Image Available</p>
            @endif
